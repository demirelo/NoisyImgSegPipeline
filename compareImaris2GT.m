...
allGTFiles = dir('*_label.mat');
for ii=1:length(allGTFiles)
    gt_filename = allGTFiles(ii).name;
    I_gt = imcomplement(read3D(gt_filename));
    L_gt = I_gt;
    ...
    %% Now, check the Imaris results
    cd('/Users/demirelo/Documents/Work/TestData/batchTest/results/labelImages/');
    allImarisFiles = dir('*_dat.tif');
    numImarisFiles = length(allImarisFiles);
    seg_score = zeros(numImarisFiles,1);
    total_seg_score = zeros(numImarisFiles,1);
    hausdorff = zeros(numImarisFiles,1)+Inf;
    for jj=1:length(allImarisFiles)
        % compute the SEG score
        L_imaris = read3D(allImarisFiles(jj).name);
        if ndims(L_imaris)==3
